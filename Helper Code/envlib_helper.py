# -*- coding: utf-8 -*-
#--------------------------------------------------------------------------------#
# A collection of wrapper functions to help interact with tethysts
# @ author: Jiawei Zhang
#--------------------------------------------------------------------------------#
...
def get_all_fenz_data_from_dataset(ts, owner, method, product_code,var_list = ["precipitation","relative_humidity","temperature","wind_speed"],squeeze_dims=True,**kw):
    datasets=ts.datasets
    dataset_info_list=[d for d in datasets if d["owner"] == owner and d["method"]==method and d["product_code"] == product_code and d["parameter"] in var_list]
    dataset_list = list()
    for dataset_info in dataset_info_list:
        station_info_list=ts.get_stations(dataset_info["dataset_id"])
        station_id_list = [station_info['station_id'] for station_info in station_info_list if station_info["name"] != "Crofoot" ] # temporarily remove "Crofoot" because of the data mess up error
        dataset_list.append(ts.get_bulk_results(dataset_info["dataset_id"],station_id_list,output='Dataset',squeeze_dims=squeeze_dims,**kw))
    output_dataset=xr.merge(dataset_list,compat="override")
    return output_dataset.transpose('time',...) # make sure that time is always the first dimension

def get_all_data_from_dataset(ts, owner, method, product_code,var_list = ["precipitation","relative_humidity","temperature","wind_speed"],squeeze_dims=True,**kw):
    ### similar as get_all_fenz_data_from_dataset but universal except for FENZ
    datasets=ts.datasets
    dataset_info_list=[d for d in datasets if d["owner"] == owner and d["method"]==method and d["product_code"] == product_code and d["parameter"] in var_list]
    dataset_list = list()
    for dataset_info in dataset_info_list:
        station_info_list=ts.get_stations(dataset_info["dataset_id"])
        station_id_list = [station_info['station_id'] for station_info in station_info_list] # temporarily remove "Crofoot" because of the data mess up error
        dataset_list.append(ts.get_bulk_results(dataset_info["dataset_id"],station_id_list,output='Dataset',squeeze_dims=squeeze_dims,**kw))
    output_dataset=xr.merge(dataset_list,compat="override")
    return output_dataset.transpose('time',...) # make sure that time is always the first dimension